    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'DM Sans','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{maxWidth:940,margin:"0 auto",padding:"32px 20px 60px"}}>
        ...
        {step===2&&(()=>{
          ...
          const touched={
            kcal:profile.kcal!==2500, cook:profile.cook!==3, out:profile.out!==2,
            sport:profile.sport!==0, cinema:profile.cinema!==0, smoke:profile.smoke!==0,
            cloth:profile.cloth!==30, buf:profile.buf!==5,
            housing:housing!=="room"||zone!=="outside",
            ...(showUkP?{ukP:ukP!==true}:{}),
            ...(showNl30?{nl30:nl30!==false}:{}),
          };
          const fields=Object.keys(touched);
          const nFilled=fields.filter(k=>touched[k]).length;
          ...
          return(
            <div className="fu" style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div className="card" style={{padding:"11px 16px",display:"flex",flexWrap:"wrap",gap:10,alignItems:"center"}}>
                ...
                {Object.entries(ZL).map(([k,v])=><button key={k} className={`chip ${zone===k?"a1":""}`} onClick={()=>setZone(k)}>{v}</button>)}
                <div className="divider"/>
                {showUkP&&<label className="toggle"><input type="checkbox" checked={ukP} onChange={e=>setUkP(e.target.checked)}/>UK pension (5%)</label>}
                {showNl30&&<label className="toggle"><input type="checkbox" checked={nl30} onChange={e=>setNl30(e.target.checked)}/>NL 30% ruling</label>}
              </div>

              {/* 2-col: fields + character — equal width */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,alignItems:"start"}}>
                ...
                <div className="card">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                    ...
                    <div style={{display:"flex",gap:4}}>
                      {fields.map((k,i)=>(
                        <div key={i} style={{width:7,height:7,borderRadius:"50%",background:touched[k]?accOn:"#1c1c38",transition:"background .3s"}}/>
                      ))}
                    </div>
                  </div>
                ...
                {/* SVG Character panel */}
                <div style={{background:"#0a0a1a",borderRadius:12,border:`1px solid ${C.border}`,padding:"24px 16px",display:"flex",flexDirection:"column",alignItems:"center",gap:14,position:"sticky",top:20}}>
                  ...
                  <svg viewBox="0 0 160 235" width="200" height="294" xmlns="http://www.w3.org/2000/svg">
                    <g>
