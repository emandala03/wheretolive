...
function calcPersonaCosts(cityKey, profile) {
  ...
  const { kcal=2500, cook=3, out=2, sport=0, cinema=0, smoke=0, cloth=30, buf=5 } = profile;
  const kf = kcal/2500;
  ...
  const misc     = 50 + gym + smoking + cloth;
  ...
  return { grocery, social, gym, smoking, cloth, misc, transport, utils, internet, buffer, total:sub+buffer };
}
...
function CharFigure({ savRate, profile={} }) {
  ...
  const labels=["thriving","doing fine","balanced","tight","struggling"];
  ...
  const hasGym=profile.sport===2, hasCin=profile.cinema>0;
  return (
...
function useSubtleCursor() {
  useEffect(() => {
    if (window.matchMedia("(hover: none)").matches) return;
    const dot = document.createElement("div");
    dot.style.cssText = `position:fixed;top:0;left:0;width:5px;height:5px;background:#818cf8;border-radius:50%;pointer-events:none;z-index:99999;transform:translate(-50%,-50%);transition:transform .15s ease,opacity .15s ease;will-change:transform;`;
    document.body.appendChild(dot);
    document.body.style.cursor = "none";
    let mx = -999, my = -999, raf;
    ...
    raf = requestAnimationFrame(tick);
    const onEnter = () => { dot.style.transform = "translate(-50%,-50%) scale(2.5)"; dot.style.opacity = ".5"; };
    const onLeave = () => { dot.style.transform = "translate(-50%,-50%) scale(1)"; dot.style.opacity = "1"; };
    const attach = () => {
    ...
    return () => {
      ...
      if (dot.parentNode) document.body.removeChild(dot);
    };
  }, []);
}
...
export default function App() {
  ...
  const [profile,setProfile]=useState({kcal:2500,cook:3,out:2,sport:0,cinema:0,smoke:0,cloth:30,buf:5});
  const [interacted,setInteracted]=useState(new Set());
  const touch=useCallback((...keys)=>setInteracted(prev=>{const n=new Set(prev);keys.forEach(k=>n.add(k));return n;}),[]);
  const updP=useCallback((patch)=>{setProfile(p=>({...p,...patch}));touch(...Object.keys(patch));},[touch]);
  ...
  const CityPicker=({label,city,pickFn,sal,setSal,pair,idx})=>(
    <div className="card" style={{borderColor:pair.accentBorder}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <label className="mono" style={{fontSize:10,color:C.textDim,whiteSpace:"nowrap"}}>Gross ({CITIES[city].currency})</label>
  ...
  const OptionsBar=()=>(
    <div className="card" style={{marginBottom:16,padding:"11px 16px",display:"flex",flexWrap:"wrap",gap:10,alignItems:"center"}}>
      ...
      <span className="mono" style={{fontSize:9,color:C.textDim,letterSpacing:2,textTransform:"uppercase"}}>Area</span>
      {Object.entries(ZL).map(([k,v])=><button key={k} className={`chip ${zone===k?"a1":""}`} onClick={()=>setZone(k)}>{v}</button>)}
      {(c1==="london"||c2==="london")&&<><div className="divider"/><label className="toggle"><input type="checkbox" checked={ukP} onChange={e=>setUkP(e.target.checked)}/>UK pension (5%)</label></>}
      {(c1==="amsterdam"||c2==="amsterdam")&&<><div className="divider"/><label className="toggle"><input type="checkbox" checked={nl30} onChange={e=>setNl30(e.target.checked)}/>NL 30% ruling</label></>}
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'DM Sans','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{maxWidth:940,margin:"0 auto",padding:"32px 20px 60px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:24,flexWrap:"wrap"}}>
          <StepPill i={0} label="Choose cities"/>
          <span style={{color:C.textDim,fontSize:11}}>›</span>
          <StepPill i={1} label="Build your profile"/>
          <span style={{color:C.textDim,fontSize:11}}>›</span>
          <StepPill i={2} label="Compare"/>
        </div>
        ...
        {step===1&&(
          <div className="fu" style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <p style={{fontSize:11,color:C.textDim,margin:0}}>Next: tell us about your lifestyle to personalise costs</p>
              <button className="tab active" onClick={()=>setStep(2)} style={{padding:"10px 28px",borderRadius:10,fontSize:13}}>Continue → build your profile</button>
            </div>
          </div>
        )}
        ...
        {/* ═══ STEP 2 ═══ */}
        {step===2&&(()=>{
          const cookOptions=["Always out","Mostly out","A mix","Mostly home","Always home"];
          const showUkP=c1==="london"||c2==="london";
          const showNl30=c1==="amsterdam"||c2==="amsterdam";
          const touchedKeys=["kcal","cook","out","sport","cinema","smoke","cloth","buf","housing",
            ...(showUkP?["ukP"]:[]), ...(showNl30?["nl30"]:[])];
          const nFilled=touchedKeys.filter(k=>interacted.has(k)).length;
          const total=touchedKeys.length;
          const accOn="#6366f1"; const accDim="#818cf8";
          const showApron=profile.cook>=4; const showFork=profile.cook<=2;
          const showBeer=profile.out>=3; const showDumbbell=profile.sport===2;
          const showTicket=profile.cinema>0;
          const showSmoke=profile.smoke>0; const showShield=profile.buf>=10;
          const charMsg=nFilled===0?"Fill in your profile to build your character":nFilled<Math.ceil(total/2)?"Keep going…":nFilled<total?"Almost there!":"Your character is complete!";
          return(
            <div className="fu" style={{display:"flex",flexDirection:"column",gap:14}}>

              {/* Housing bar — moved from step 1 */}
              <div className="card" style={{padding:"11px 16px",display:"flex",flexWrap:"wrap",gap:10,alignItems:"center"}}>
                <span className="mono" style={{fontSize:9,color:C.textDim,letterSpacing:2,textTransform:"uppercase"}}>Housing</span>
                {Object.entries(HL).map(([k,v])=><button key={k} className={`chip ${housing===k?"a1":""}`} onClick={()=>{setHousing(k);touch("housing");}}>{v}</button>)}
                <div className="divider"/>
                <span className="mono" style={{fontSize:9,color:C.textDim,letterSpacing:2,textTransform:"uppercase"}}>Area</span>
                {Object.entries(ZL).map(([k,v])=><button key={k} className={`chip ${zone===k?"a1":""}`} onClick={()=>{setZone(k);touch("housing");}}>{v}</button>)}
                <div className="divider"/>
                {showUkP&&<label className="toggle"><input type="checkbox" checked={ukP} onChange={e=>{setUkP(e.target.checked);touch("ukP");}}/>UK pension (5%)</label>}
                {showNl30&&<label className="toggle"><input type="checkbox" checked={nl30} onChange={e=>{setNl30(e.target.checked);touch("nl30");}}/>NL 30% ruling</label>}
              </div>

              {/* 2-col: fields + character — equal width, stretched */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,alignItems:"stretch"}}>

                {/* Profile fields */}
                <div className="card">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                    <p className="mono" style={{fontSize:9,color:C.accent,letterSpacing:2,textTransform:"uppercase",margin:0}}>Your lifestyle</p>
                    <div style={{display:"flex",gap:4}}>
                      {touchedKeys.map((k,i)=>(
                        <div key={i} style={{width:7,height:7,borderRadius:"50%",background:interacted.has(k)?accOn:"#1c1c38",transition:"background .3s"}}/>
                      ))}
                    </div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:14}}>

                    <FieldRow label="Daily calories" value={`${profile.kcal} kcal`}>
                    ...
                    <FieldRow label="Eating habits">
                      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                        {cookOptions.map((lbl,i)=>(
                          <button key={i} className={`chip ${profile.cook===i+1?"a1":""}`} onClick={()=>updP({cook:i+1})} style={{fontSize:10,flex:1,textAlign:"center",minWidth:60}}>{lbl}</button>
                        ))}
                      </div>
                    </FieldRow>

                    <FieldRow label="Evenings out / week" value={`${profile.out}×`}>
                    ...
                    <FieldRow label="Sport">
                      <ToggleGroup field="sport" options={["None","Outdoor sport","Gym"]}/>
                    </FieldRow>

                    <FieldRow label="Cinema">
                      <ToggleGroup field="cinema" options={["Never","Once a month","2+× a month"]}/>
                    </FieldRow>

                    <FieldRow label="Smoking">
                      <ToggleGroup field="smoke" options={["Non-smoker","Occasionally","Regularly"]}/>
                    </FieldRow>

                    <FieldRow label="Clothing budget" value={`€${profile.cloth}/mo`}>
                      <input type="range" min="20" max="200" step="10" value={profile.cloth} onChange={e=>updP({cloth:+e.target.value})} style={{width:"100%"}}/>
                    </FieldRow>

                    <FieldRow label="Emergency buffer" value={`${profile.buf}%`}>
                ...
                {/* SVG Character panel */}
                <div style={{background:"#0a0a1a",borderRadius:12,border:`1px solid ${C.border}`,padding:"24px 16px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14}}>
                  <span className="mono" style={{fontSize:8,color:C.textDim,letterSpacing:2,textTransform:"uppercase"}}>{nFilled} / {total} fields set</span>

                  <svg viewBox="0 0 160 235" style={{width:"100%",maxWidth:260,height:"auto"}} xmlns="http://www.w3.org/2000/svg">
                    <g>
                      <rect x="57" y="168" width="18" height="55" rx="7" fill="#13132a" stroke="#1c1c38" strokeWidth="1.5"/>
                      <rect x="85" y="168" width="18" height="55" rx="7" fill="#13132a" stroke="#1c1c38" strokeWidth="1.5"/>
                      <line x1="57" y1="106" x2="26" y2="150" stroke="#1c1c38" strokeWidth="11" strokeLinecap="round"/>
                      <line x1="103" y1="106" x2="134" y2="150" stroke="#1c1c38" strokeWidth="11" strokeLinecap="round"/>
                      <rect x="47" y="92" width="66" height="80" rx="12" fill="#13132a" stroke="#1c1c38" strokeWidth="1.5"/>
                      <circle cx="80" cy="58" r="32" fill="#13132a" stroke="#1c1c38" strokeWidth="1.5"/>
                    </g>
                    <g>
                      <line x1="57" y1="106" x2="26" y2="150" stroke="#1e1a40" strokeWidth="9" strokeLinecap="round"/>
                      <line x1="103" y1="106" x2="134" y2="150" stroke="#1e1a40" strokeWidth="9" strokeLinecap="round"/>
                      <rect x="58" y="168" width="16" height="53" rx="6" fill="#1a1838"/>
                      <rect x="86" y="168" width="16" height="53" rx="6" fill="#1a1838"/>
                      <rect x="48" y="93" width="64" height="78" rx="11" fill="#151328"/>
                      <rect x="67" y="93" width="26" height="12" rx="4" fill="#1c1c38"/>
                      <circle cx="80" cy="58" r="31" fill="#1a1838"/>
                      <circle cx="71" cy="53" r="3.5" fill={accDim}/>
                      <circle cx="89" cy="53" r="3.5" fill={accDim}/>
                      <circle cx="71" cy="53" r="1.5" fill="#080812"/>
                      <circle cx="89" cy="53" r="1.5" fill="#080812"/>
                      <path d="M73 68 Q80 75 87 68" stroke={accDim} strokeWidth="1.8" fill="none" strokeLinecap="round"/>
                      <path d="M62 36 Q80 22 98 36" stroke="#6366f180" strokeWidth="5" fill="none" strokeLinecap="round"/>
                    </g>
                    {showApron&&<g>
                      <path d="M58 105 L58 168 Q80 175 102 168 L102 105 Q80 112 58 105Z" fill="#6366f112" stroke="#6366f140" strokeWidth="1"/>
                      <path d="M67 93 L65 105 Q80 112 95 105 L93 93" stroke="#6366f150" strokeWidth="1.5" fill="none"/>
                      <rect x="68" y="135" width="24" height="16" rx="4" fill="#6366f118" stroke="#6366f140" strokeWidth="1"/>
                    </g>}
                    {showFork&&<g transform="translate(122,142)">
                      <line x1="0" y1="-18" x2="0" y2="8" stroke={accDim} strokeWidth="2" strokeLinecap="round"/>
                      <line x1="-4" y1="-18" x2="-4" y2="-6" stroke={accDim} strokeWidth="2" strokeLinecap="round"/>
                      <line x1="4" y1="-18" x2="4" y2="-6" stroke={accDim} strokeWidth="2" strokeLinecap="round"/>
                      <path d="M-4 -6 Q0 -2 4 -6" stroke={accDim} strokeWidth="2" fill="none" strokeLinecap="round"/>
                    </g>}
                    {showBeer&&<g transform="translate(10,140)">
                      <path d="M5 0 L3 26 L21 26 L19 0Z" fill="#34d39918" stroke="#34d39960" strokeWidth="1.2"/>
                      <rect x="3" y="0" width="18" height="7" rx="2" fill="#34d39930"/>
                      <path d="M19 6 Q28 6 28 13 Q28 20 19 20" stroke="#34d39960" strokeWidth="1.8" fill="none" strokeLinecap="round"/>
                      <circle cx="9" cy="16" r="2" fill="#34d39930"/>
                      <circle cx="14" cy="10" r="1.5" fill="#34d39930"/>
                    </g>}
                    {showDumbbell&&<g transform="translate(13,95)">
                      <rect x="0" y="5" width="30" height="6" rx="3" fill="#9090b8"/>
                      <rect x="0" y="2" width="9" height="12" rx="4" fill={accDim}/>
                      <rect x="21" y="2" width="9" height="12" rx="4" fill={accDim}/>
                    </g>}
                    {showTicket&&<g transform="translate(106,162)">
                      <rect x="0" y="0" width="36" height="22" rx="5" fill="#fbbf2415" stroke="#fbbf2450" strokeWidth="1"/>
                      <line x1="13" y1="0" x2="13" y2="22" stroke="#fbbf2430" strokeWidth="1" strokeDasharray="3 2"/>
                      <circle cx="24" cy="8" r="3" fill="#fbbf2460"/>
                    </g>}
                    {showSmoke&&<g>
                      <rect x="92" y="63" width="24" height="6" rx="3" fill="#6b6b8a"/>
                      <rect x="92" y="63" width="7" height="6" rx="3" fill="#f87171"/>
                      <path d="M101 60 Q99 53 103 47" stroke="#9090b840" strokeWidth="1.5" fill="none" strokeLinecap="round"/>
                    </g>}
                    {showShield&&<g transform="translate(68,90)">
                      <path d="M12 0 L24 4 L24 14 Q24 22 12 26 Q0 22 0 14 L0 4Z" fill="#34d39912" stroke="#34d39950" strokeWidth="1"/>
                      <text x="12" y="16" fontFamily="DM Mono,monospace" fontSize="8" fill="#34d399" textAnchor="middle" fontWeight="500">%</text>
                    </g>}
                  </svg>

                  <p style={{fontSize:11,color:C.textDim,textAlign:"center",margin:0,lineHeight:1.6}}>{charMsg}</p>
                </div>
              </div>

              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <button className="tab" onClick={()=>setStep(1)} style={{fontSize:12}}>← Back to cities</button>
                <button className="tab active" onClick={()=>setStep(3)} style={{padding:"10px 28px",borderRadius:10,fontSize:13}}>See the comparison →</button>
              </div>
            </div>
          );
        })()}
        ...
        {step===3&&(
          <div className="fu">
            <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
              <button className={`tab ${view==="compare"?"active":""}`} onClick={()=>setView("compare")}>Compare cities</button>
              <button className={`tab ${view==="ranking"?"active":""}`} onClick={()=>setView("ranking")}>Ranking</button>
              <div style={{flex:1}}/>
              <button className="tab" onClick={()=>setStep(2)} style={{fontSize:11,padding:"6px 14px"}}>← Edit profile</button>
            </div>
                    ...
                    <div key={i}>
                      <div className="card" style={{borderColor:pair.accentBorder}}>
                        ...
                        <div style={{borderTop:`1px solid ${C.border}`,marginTop:10,paddingTop:8}}>
                          ...
                          {[
                            ...
                            ...(r.persona?[
                              {l:"Groceries",          v:"€"+fN(r.persona.grocery)},
                              {l:"Social life",        v:"€"+fN(r.persona.social)},
                              {l:"Sport + leisure",    v:"€"+fN(r.persona.gym+r.persona.smoking+r.persona.streamC)},
                              {l:"Clothing + misc",    v:"€"+fN(r.persona.cloth+50)},
                              {l:"Transport",          v:r.persona.transport===0?"Free":"€"+fN(r.persona.transport)},
                              {l:"Utilities + internet",v:"€"+fN(r.persona.utils+r.persona.internet)},
                            ]:[
                      ...
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginTop:12,paddingTop:14,borderTop:`1px solid ${C.border}`}}>
                        ...
                        <div className="mono" style={{fontSize:9,color:C.textDim,letterSpacing:1.5,textTransform:"uppercase",marginTop:8}}>
                          {r.disp>0?`€${fN(r.disp)}/mo surplus`:`€${fN(Math.abs(r.disp))}/mo deficit`}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div className="card">
                  <p className="mono" style={{fontSize:9,color:C.accent,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>
                    Disposable income — {HL[housing].toLowerCase()}, {ZL[zone].toLowerCase()} — personalised profile
                  </p>
