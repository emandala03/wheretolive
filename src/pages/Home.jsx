...
function ASCIIHero() {
  ...
  const stateRef     = useRef({ grid: [], litIdx: [], ghostIdx: new Set(), cols: 0, rows: 0 });
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      stateRef.current = { grid, litIdx, ghostIdx: new Set(), cols, rows };
      renderGrid();
    };
    ...
    // Background flicker — 3% of lit chars per tick, also decays ghost chars
    const flicker = () => {
      const { grid, litIdx, ghostIdx } = stateRef.current;
      if (litIdx.length > 0) {
        // Flicker lit chars
        ...
        // Decay ghost chars (empty cells that got spray) back to space
        if (ghostIdx.size > 0) {
          const ghosts = [...ghostIdx];
    ...
    // Mouse distortion — large blast + spray into empty areas
    const RADIUS      = 160;
    const SPRAY_R     = 90;   // how far chars spray into empty space

    const onMouseMove = (e) => {
      const { grid, litIdx, ghostIdx, cols, rows } = stateRef.current;
      ...
      const r1 = Math.min(rows - 1, Math.ceil((my + RADIUS) / lh));
      ...
      for (let r = r0; r <= r1; r++) {
        for (let c = c0; c <= c1; c++) {
          ...
          if (grid[pos] !== " ") {
            // Lit char — blast probability: ~100% within 40px, falls off sharply
            if (dist < RADIUS) {
              const prob = dist < 40 ? 1.0 : Math.pow(1 - (dist - 40) / (RADIUS - 40), 1.4);
              if (Math.random() < prob) grid[pos] = randChar();
            }
          } else if (dist < SPRAY_R) {
            // Empty area — spray ghost chars with probability peaking at centre
            const prob = Math.pow(1 - dist / SPRAY_R, 2.2) * 0.65;
            if (Math.random() < prob) {
              grid[pos] = randChar();
              ghostIdx.add(pos);
            }
  ...
  return (
    <div style={{
      ...
      <section className="dot-bg" style={{
        ...
        {/* ":/" mark — top right */}
        <span className="fi3 hero-br" style={{
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <h2 style={{ fontSize: "clamp(32px,5vw,60px)", letterSpacing: -2, lineHeight: .9, fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>
              THREE<br/><span style={{ color: C.dimMid, fontWeight: 300 }}>STEPS.</span>
            </h2>
          </div>
        ...
        <div style={{ marginTop: 40 }}>
          {[
            ...
            <div key={i} className="step-row">
              ...
              <div>
                <div style={{ fontSize: "clamp(14px,1.5vw,18px)", marginBottom: 8, letterSpacing: -.3, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>{s.title}</div>
                <div style={{ fontSize: 13, color: C.dimMid, lineHeight: 1.7, maxWidth: 480, fontFamily: "'DM Sans', sans-serif" }}>{s.body}</div>
              </div>
            </div>
          ))}
      ...
      <section style={{
        ...
        <div>
          ...
          <p style={{ fontSize: "clamp(28px,4vw,52px)", letterSpacing: -2, lineHeight: .9, fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>
            SEE WHERE YOUR<br/><span style={{ color: C.dimMid, fontWeight: 300 }}>MONEY GOES.</span>
          </p>
        </div>
