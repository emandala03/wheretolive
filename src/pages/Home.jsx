...
function ASCIIHero() {
  ...
  const canvasRef    = useRef(null);   // off-screen sampling canvas
  ...
  useEffect(() => {
    ...
    // Character cell dimensions — measured once after font is loaded
    let CHAR_W = 7.2;
    const LINE_H = 14;
    ...
    const build = () => {
      ...
      // ── Off-screen canvas: real pixel dimensions = W × H ──────────────
      // We draw text here, then sample every CHAR_W × LINE_H block
      ...
      // Find font-size that makes the text fill ~88% of the width
      // for the WIDER of the two lines
      let fs = 120;
      const tryFit = () => {
      ...
      // ── Sample: one sample per character cell ─────────────────────────
      // Sample at the CENTER of each cell (x + CHAR_W/2, y + LINE_H/2)
    ...
    // Flicker: replace ~3% of lit chars per tick
    const flicker = () => {
